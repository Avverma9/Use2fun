...
function ViewVip() {
  ...
  const handleConfirmDelete = async () => {
    if (selectedItem !== null) {
      ...
      try {
        const response = await fetch(`https://use2fun.onrender.com/admin/vip/delete/${selectedItemData._id}`, {
          method: 'DELETE',
        });
  ...
  const handleUpdateSubmit = async () => {
    ...
    try {
      const response = await fetch(`https://use2fun.onrender.com/admin/vip/update/${editedData._id}`, {
        method: 'PUT',
        body: formDataToSend,
      });
          ...
          {data && data.length > 0 ? (
            data.map((item, index) => (
              <tr key={index} className="row2">
                ...
                <td className="price">
                  <img className='images' src={item.images[0]} alt="image" />
                </td>
                <td className="price">{item.price}</td>
                <td className="price">{item.level}</td>
                <td className="price">{item.day} days</td>
                <td>
